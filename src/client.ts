...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['WIKIMEDIA_ENTERPRISE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['WIKIMEDIA_ENTERPRISE_LOG'] or 'warn' if it isn't set.
   */
...
export class WikimediaEnterprise {
  ...
  constructor({
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.enterprise.wikimedia.com`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? WikimediaEnterprise.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('WIKIMEDIA_ENTERPRISE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.enterprise.wikimedia.com';
  }
   ...
   * Metadata
   */
  codes: API.Codes = new API.Codes(this);
  /**
   * Metadata
   */
  ...
  wikidata: API.Wikidata = new API.Wikidata(this);
  /**
...
WikimediaEnterprise.Languages = Languages;
WikimediaEnterprise.Projects = Projects;
WikimediaEnterprise.Namespaces = Namespaces;
WikimediaEnterprise.Batches = Batches;
WikimediaEnterprise.Snapshots = Snapshots;
